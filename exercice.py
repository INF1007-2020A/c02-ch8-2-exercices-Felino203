...
def convert_to_bytes(samples):
	...
	for sample in samples:
		int_sample = sample * MAX_SAMPLE_VALUE
		data += struct.pack("h", bytes([int_sample]))
	return data
...
def convert_to_samples(bytes):
	...
	for i in range(0, len(bytes), 2):
		...
		integer_sample = struct.unpack("h", encoded_sample)
...
def main():
	...
	with wave.open("data/stravinsky.wav", "rb") as reader:
		data = reader.readframes(reader.getframes())
		samples = convert_to_samples(data)
		
		data = convert_to_bytes(samples)
		with wave.open("data/stravinsky_mod.wav", "wb") as writer:
			writer.setnchannels(2)
			writer.setsampwidth(2)
			writer.setframerate(SAMPLING_FREQ)
			writer.setnframes(SAMPLING_FREQ * 5)
